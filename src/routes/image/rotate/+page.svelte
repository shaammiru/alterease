<script>
	...
	const handleRotate = async () => {
		...
		try {
			const response = await fetch(`${PUBLIC_API_URL}/image/rotate`, {
				method: 'POST',
				body: formData
			});
...
<svelte:head><title>Alterease | Image Rotate</title></svelte:head>

<div class="min-h-screen text-center">
	<div class="m-10">
		<h1 class="font-bold text-xl">Image Rotate</h1>
	</div>
	...
	<div class="p-4 space-x-2 flex justify-center">
		<table class="table-auto">
			<tr>
				<td class="p-2">
					<label for="width">Rotation Degree</label>
				</td>
				<td class="p-2">
					<input
						id="width"
	...
	<div class="p-4">
		{#if !isRotated}
			{#if fileSelected}
				...
				{:else}
					<button class="btn btn-wide btn-accent" on:click={handleRotate}>Rotate</button>
				{/if}
			{:else}
				<button class="btn btn-wide btn-disabled">Rotate</button>
			{/if}
		{:else}
			<div class="p-2">
